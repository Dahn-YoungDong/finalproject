...
#left out snps_fil_hwe_low
```

```{r}
#C. Choose one of the datasets (A or B): --- choose B: snps_fil_hwe_high
...
```{r}
#D. Define R² value: #how to properly assign this?
r2 = 0.4
```
...
#ld_within <- Linkage(snps_fil_hwe, type="geno-r2", linkageOptions(min.r2=r2)) # Nov 29th, 2020 # seem nothing to be removed. No SNP deviating from HW equilibrium within each pop. I guess because there is only one sample for each population.
...
#ld_between <- Linkage(snps_fil_ld, type="interchrom-geno-r2", linkageOptions(min.r2=r2)) #maube change r2 value here, to leave out more SNPs
...
#A. Quality indexes: #important is to know how to evaluate these indices
site.depth2 <- Query(snps_fil_ldF, "site-mean-depth")
quality2 <- Query(snps_fil_ldF, "site-quality") #no site quality info
...
#Estimate ancestry coefficients-- estimation of spatial population structure
set.seed(13)
...
#B. Select FST statistics for the best run ## how to select here? There are 100 results
FST = res$Fst
...
```{r}
#D. Compute adjusted p-values from the combined z-scores and plot histogram of p-values # was not successful
n = dim(Q(project, run, optimal_K_sNMF))[1] # this requires running "2.filter_supp", used optimal K for sNMF method, instead of TESS method ### this can be really wrong ....
z.scores = sqrt(FST*(n-optimal_K)/(1-FST)) #and here I didn't change back for optimal_K... really nonsense
...
neutral_after_fst_TESS #"16 individuals and 100 SNPs."

## Nothing is excluded- from C_fst object. the 100SNPS are still here 
```
